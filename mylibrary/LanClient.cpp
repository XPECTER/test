...
bool CLanClient::Connect(wchar_t *szFileName)
{
	if (true == this->_bConnected)
		return true;

	if (!this->SetNetworkConfig(szFileName))
		return false;
...
bool CLanClient::SetNetworkConfig(wchar_t *szFileName)
{
	CConfigParser parser;
	wchar_t szBlock[32] = { 0, };
	wchar_t szKey[32] = { 0, };

	if(!parser.OpenConfigFile(szFileName))
	{
		SYSLOG(L"SYSTEM", LOG::LEVEL_ERROR, L"Config file %s open failed", szFileName);
		return false;
	}
