...
#define errstr_STARTUP L"WSAStartup error"
#define errstr_CREATE_SOCKET L"Create Socket error."
#define errstr_CREATE_IOCP L"Create IOCP error"
#define errstr_CREATE_THREAD_ACCEPT L"Create Accept thread error"
#define errstr_CREATE_THREAD_WORKER L"Create Worker thread error"
#define errstr_CREATE_THREAD_MONITOR L"Create Monitor thread error"
#define errstr_REGIST_IOCP L"Regist IOCP error"
#define errstr_REGIST_ARRAY L"Array Full"
#define errstr_SOCKET_OPT_NAGLE L"Socket Option : Nagle Error"
#define errstr_SOCKET_OPT_LINGER L"Socket Option : Linger Error"
#define errstr_SOCKET_BIND L"Bind error"
#define errstr_SOCKET_LISTEN L"Listen error"
#define errstr_SOCKET_WSASEND L"WSASend Error"
#define errstr_SOCKET_WSARECV L"WSARecv Error"
#define errstr_SOCKET_IOFAIL L"IO Fail."
#define errstr_QUEUE_MOVEWRITE L"MoveWrite Error"
#define errstr_QUEUE_REMOVEDATA L"RemoveData Error"
...
bool CNetServer::Start(WCHAR *szIP, int iPort, int iThreadNum, bool bNagleEnable, int iMaxUser)
{
	...
	{
		OnError(WSAGetLastError(), -1, errstr_STARTUP);
		return false;
	}
	...
	{
		OnError(WSAGetLastError(), -1, errstr_CREATE_SOCKET);
		return false;
	}
	...
	{
		OnError(WSAGetLastError(), -1, errstr_SOCKET_OPT_LINGER);
	}
	...
	if (true == bNagleEnable)
	{
		...
		{
			OnError(WSAGetLastError(), -1, errstr_SOCKET_OPT_NAGLE);
			return false;
		}
	}
	...
	{
		OnError(WSAGetLastError(), 0, errstr_SOCKET_BIND);
		return false;
	}
...
bool CNetServer::Init_Thread(int _iThreadNum)
{
	...
	{
		OnError(GetLastError(), -1, errstr_CREATE_THREAD_ACCEPT);
		return FALSE;
	}
	...
	for (int i = 0; i < _iWorkerThreadNum; ++i)
	{
		...
		{
			OnError(GetLastError(), -1, errstr_CREATE_THREAD_WORKER);
			return FALSE;;
		}
	}
...
unsigned _stdcall CNetServer::AcceptThreadFunc(void *lpParam)
{
	...
	{
		pServer->OnError(WSAGetLastError(), -1, errstr_SOCKET_LISTEN);
		return -1;
		// 서버 죽여야지
	}
	...
	while (true)
	{
		...
		{
			pServer->OnError(GetLastError(), -1, errstr_REGIST_IOCP);
	...
	while (true)
	{
		...
		else
		{
			...
			else if (tempOv == &pSession->RecvOverlap)
			{
					...
					pServer->OnError(0, pSession->ClientId, errstr_QUEUE_MOVEWRITE);
